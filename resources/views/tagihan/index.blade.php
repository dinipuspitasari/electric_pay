...
@section('content')
    ...
    <div class="container mb-10">
        ...
        <div class="col">
            <h2 class="mt-2 text-xl mb-1 font-semibold"> Data Tagihan</h2>
            <hr>
        </div>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-900 px-10 ">
                <thead class="text-xs text-white uppercase bg-gray-900 h-8">
                    <tr align="center">
                        ...
                        <th class="border border-gray-300 px-2 py-2">Periode</th>
                        <th class="border border-gray-300 px-2 py-2">Meter</th>
                        <th class="border border-gray-300 px-2 py-2">Jumlah Meter</th>
                ...
                <tbody>
                    @foreach ($tagihan as $u)
                        <tr align="center" class="bg-white border-b text-gray-900">
                            ...
                            <td class="border border-gray-300 px-2 py-2">{{ $u->pelanggan->user->name }}</td>
                            <td class="border border-gray-300 px-2 py-2">{{ $u->pelanggan->user->email }}</td>
                            <td class="border border-gray-300 px-2 py-2">{{ $u->pelanggan->nomor_kwh }}</td>
                            <td class="border border-gray-300 px-2 py-2">{{ getMonthNames($u->bulan) }} {{ $u->tahun }}</td>
                            <td class="border border-gray-300 px-2 py-2">{{ $u->meter_awal }} - {{ $u->meter_akhir }}</td>
                            <td class="border border-gray-300 px-2 py-2">{{ $u->jumlah_meter }}</td>
                            <td class="border border-gray-300 px-2 py-2 whitespace-nowrap">
                                @if ($u->status == 1)
                                <div class="bg-red-200 p-1 rounded-full text-red-600">Menunggu Pembayaran</div>
                                @elseif ($u->status == 2)
                                <div class="bg-green-200 p-1 rounded-full text-green-600">Sudah Bayar</div>
                                @endif
                            </td>
                            <td>{{ $u->created_at }}</td>
