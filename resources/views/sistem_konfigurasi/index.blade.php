...
@section('content')
    ...
    <div class="container mb-10">
            ...
            <div class="flex md:flex-row flex-col-reverse gap-y-2 md:gap-y-0 md:justify-between items-center">
                <div class="relative w-full">
                    ...
                    <input type="text" id="table-search"
                        class="block pt-2 w-full md:w-auto ps-10 text-sm text-gray-900 border border-gray-300 rounded-full bg-white focus:ring-blue-300 focus:border-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-yellow-30000 dark:focus:border-yellow-30000"
                        placeholder="Cari">
                </div>
            </div>
        ...
        <div class="col">
            <h2 class="mt-2 text-xl mb-1 font-semibold">Sistem Konfigurasi</h2>
            <hr>
        </div>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-900 px-10 ">
                <thead class="text-xs text-white uppercase bg-gray-900 h-8">
                    <tr align="center">
                        ...
                        <th class="border border-gray-300 px-2 py-2">Nama Konfigurasi</th>
                        <th class="border border-gray-300 px-2 py-2">Nilai</th>
                        <th class="border border-gray-300 px-2 py-2">Deskripsi</th>
                        <th class="border border-gray-300 px-2 py-2">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($sistem_konfigurasi as $u)
                        <tr align="center" class="bg-white border-b text-gray-900">
                            <td class="border border-gray-300 px-2 py-2">{{ $loop->iteration }}</td>
                            <td class="border border-gray-300 px-2 py-2">{{ $u->config_name }}</td>
                            <td class="border border-gray-300 px-2 py-2">Rp. {{ $u->value }}</td>
                            <td class="border border-gray-300 px-2 py-2">{{ $u->description }}</td>
                            <td class="flex items-center justify-center py-1">
                                <a href={{ url('/sistem-konfigurasi/edit/' . $u->id) }} type="button"
